...
# Ruta base
base_path = r'C:\Users\luis-\Downloads\TFM\DatosEspectrometria\2025_03_18_espectrometro_FC_Ciencias'

# Leer I0 (referencia)
i0_file = os.path.join(base_path, 'ref_blanco_RC0_T.txt')
i0_data = pd.read_csv(i0_file, sep=r'\s+', header=None, skiprows=19).values
i0 = i0_data[:, 1]  # segunda columna

# Buscar todos los archivos de espectros menos la referencia
spectra_files = [f for f in os.listdir(base_path) if f.endswith('.txt') and 'ref_blanco_RC0_T.txt' not in f]
...
for filename in spectra_files:
    ...
    data = pd.read_csv(filepath, sep=r'\s+', header=None, skiprows=19).values
