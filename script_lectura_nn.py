...
# Leer I0 (referencia)
i0_file = os.path.join(base_path, 'referencias', 'Radio_0_spectra.txt')
i0_data = np.loadtxt(i0_file)
i0 = i0_data[:, 1]  # segunda columna

# Buscar todos los archivos de espectros menos la referencia
spectra_files = [f for f in os.listdir(base_path) if f.endswith('.txt') and 'Radio_0_spectra.txt' not in f]
